...
const MacOSDock: React.FC<MacOSDockProps> = ({
  ...
  const contentWidth = currentPositions.length > 0 ? Math.max(...currentPositions.map((pos, index) => pos + baseIconSize * currentScales[index] / 2)) : apps.length * (baseIconSize + baseSpacing) - baseSpacing;
  const padding = Math.max(8, baseIconSize * 0.12);
  return;
};
