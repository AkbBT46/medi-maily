      ...
      <div className="container-custom text-center relative">
        ...
        <h1 className={`text-4xl md:text-5xl lg:text-6xl font-bold mb-6 max-w-4xl mx-auto transition-all duration-700 ease-out ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          <TextShimmer
            as="span"
            duration={3}
            className="[--base-color:theme(colors.blue.400)] [--base-gradient-color:theme(colors.blue.200)]"
          >
          ...
          <TextShimmer
            as="span"
            duration={3}
            className="[--base-color:theme(colors.blue.300)] [--base-gradient-color:theme(colors.blue.100)]"
          >
            Mehr Patienten.
          </TextShimmer>
        </h1>
        ...
        <div className={`mb-6 flex justify-center transition-all duration-700 delay-75 ease-out ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          <MediMailyAnimation 
            key={animationKey}
            speed={1.1} 
            className="h-16 md:h-20 text-blue-400"
            onAnimationComplete={handleAnimationComplete}
          />
        </div>
        
        <p className={`text-lg md:text-xl text-slate-300 mb-6 max-w-2xl mx-auto transition-all duration-700 delay-100 ease-out ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          Helfen Sie Ihrer Zahnarztpraxis zu wachsen mit personalisierten E-Mail-Kampagnen, die mehr Patienten gewinnen und halten.
        </p>
        
        <p className={`text-lg md:text-xl font-medium text-blue-300 mb-10 transition-all duration-700 delay-200 ease-out ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>Mehr Zeit, mehr Anfragen, weniger Aufwand...</p>
        ...
        <div className={`flex flex-col sm:flex-row justify-center gap-4 transition-all duration-700 delay-300 ease-out ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          ...
          <Button variant="outline" className="btn-outline text-base py-6 border-blue-400 text-blue-300 bg-blue-900/20 hover:bg-blue-800/30 px-[38px] my-0 mx-0">
            Mehr Erfahren
          </Button>
        </div>
        ...
        <div className={`mt-16 md:mt-20 relative transition-all duration-700 delay-500 ease-out ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          <div className="bg-slate-800/60 backdrop-blur-sm rounded-lg shadow-xl p-2 md:p-4 max-w-3xl mx-auto relative z-10 hover:shadow-2xl transition-all duration-300 border border-slate-700/50">
            ...
            <div className="absolute -bottom-2 -right-2 md:-bottom-4 md:-right-4 bg-blue-900 rounded-full p-3 md:p-6 shadow-lg animate-bounce-in">
              <div className="bg-blue-600 text-white font-bold rounded-full h-16 w-16 md:h-24 md:w-24 flex items-center justify-center">
                <div>
                  <div className="text-sm md:text-base">Bis zu</div>
                  <div className="text-xl md:text-3xl">35%</div>
                </div>
              </div>
              <div className="absolute bottom-0 left-full whitespace-nowrap pl-3 font-medium text-sm md:text-base text-slate-100">
